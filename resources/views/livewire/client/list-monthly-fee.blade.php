            ...
            <tbody>
                @foreach($fees as $fee)
                    <tr>
                        ...
                        <td>{{ $fee->month }}</td>
                        <td class="font-semibold">{{ $fee->fee_student }}</td>
                        <td>
                        ...
                        <td>
                            @if($fee->status != 1)
                                <form action="{{ route('payment.create', ['report_class_id' => $fee->id]) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-primary btn-sm" @disabled($processingPayment)>
